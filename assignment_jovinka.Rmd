...
{cc_freq_selected%>%
  ...
  ) %>% layout(title = unique(cc_freq_selected$location))
}
...
mismatch_cc_loyalty = cc_loyalty %>% filter(is.na(cc_loyalty$loyaltynum)) %>% unique()
...
Based on employee location, we can figure out the timing range for cc transaction in 
...
13,15,21,24 -> meeting point 1,2,4,5,6

Inga Ferro, Loreto Bodrogi, Hennie Osvaldo, and Minke Mies are 

```{r showing unofficial relationship among gastech}
# gps_sf_stopfin_unique1 <- gps_sf_stopfin  %>% select(Possible_Location,geometry)%>%unique()%>%arrange(Possible_Location)
# 
# gps_sf_stopfin_unique <- gps_sf_stopfin_unique1[-c(221,476,644,1100,1103),]
# 
...
#   tm_shape(abila_st_2) +
#   tm_lines(col = "red", scale = 1)+
#   tm_shape(gps_path_stopfin[c(50:56),]) +
#   tm_lines(col = "blue", scale =4, interactive = TRUE) +
#  tm_text("Possible_Location", size = 3, remove.overlap = TRUE, overwrite.lines = TRUE, just = "top")
